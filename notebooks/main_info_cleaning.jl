...
# ╔═╡ 9518e509-f50e-42d3-a3d5-9f1b1eda5287
begin
  using PlutoUI
  TableOfContents()
end

# ╔═╡ 2a3005a0-c96a-11ed-2a69-cf08ee0db423
begin
	using DataFrames
...
# ╔═╡ 5190ff7c-748c-48bb-9ecf-3bfe3bd81871
md"""
# Cleaning the previous main_info dataset
"""

# ╔═╡ 9f44febf-41f8-454d-ac7d-586b81561699
# move to the project directory
cd("..")

# ╔═╡ deb29c89-3a09-460e-a44d-fe667a6127c7
pwd()

# ╔═╡ c5aeef02-f952-4a16-b33e-58d9f0c073ca
...
# ╔═╡ 24dfeb43-45f3-44ee-a285-e65d73edde8f
md"""
## Convert time zones
"""

# ╔═╡ be2d61ed-5756-4ab3-a59f-38b6681cec31
md"""
Since in my previous dataset, the time in scrape_time column is Berkeley local time, we need to convert it to Chinese time.
"""

# ╔═╡ 0d605a50-f95e-4119-addb-817f0b046ea3
...
# ╔═╡ 9de4ea12-9561-49d4-be8c-ea433b65ae4d
transform!(main_info_df,:scraped_time=>ByRow(convert_time_zone)=>:scraped_time)

# ╔═╡ 6d5eab1e-fbfb-4456-a443-713f2a386f49
md"""
## Filter projects and compute more metrics
"""

# ╔═╡ 6e3e5824-2e5e-4478-8435-526abf42457c
md"""
To track the whole lifecycle of a campaign, only keep the projects whose lifespan was covered by my web crawler.
"""

# ╔═╡ 1995c09a-f270-4533-a9a9-ac9f3829be0f
subset!(main_info_df,
[:start_time,:end_time,:scraped_time]
=>(x,y,z)->((x.>=minimum(z)).&&(y.<=maximum(z))))

# ╔═╡ a7ebb292-7345-40c2-8c5e-f46cd7efe832
unique(main_info_df[:,:project_id])|>length

# ╔═╡ e903e6ee-0719-49b9-97cd-17c0639cf66d
md"""
You can see there are now only $(unique(main_info_df[:,:project_id])|>length) projects remaining.
"""

# ╔═╡ f435431c-9480-40d6-8855-075d5f3fd825
md"""
Apart from the information directly recorded in this table, we also want to know
1. The length of each project
2. The number of periods that had passed since the project launched at the specified time
3. The outcome of the project
4. How much a project earned during each period
"""

# ╔═╡ d585b326-2164-441d-85db-b1826f5d47ca
function compute_time_difference(t1::DateTime,t2::DateTime,unit::TimePeriod)
	raw_diff = t1-t2
	return raw_diff/unit |> ceil |> Int
end

# ╔═╡ 4395806a-089a-4def-a717-c64a1d7fff24
...
# ╔═╡ 536cf2e8-94ba-4e49-b11f-437d622d31ad
md"""
Here comes a seemingly naive question: **Does a specific project have the same project_length throughout its lifespan?**
"""

# ╔═╡ 7cf64edb-3788-4b1b-81d7-3d23584ee719
@chain main_info_df begin
	groupby(_,:project_id)
	combine(_,:project_length=>(x->maximum(x)-minimum(x))=>:length_diff)
	subset(_,:length_diff=>x->x.!=0)
end

# ╔═╡ 43623613-04ad-45d1-804c-b456fba6ea6f
md"""
The answer is **not**!

After manually checking each of these projects, I found that some project ended earlier to deliver the goods earlier, while some projects delayed their termination to raise more money. And some early conclusions resulted from cancellation. Since the number of there projects are relatively small, we just remove them from the sample.
"""

# ╔═╡ 06070a89-d5a5-482f-957f-06c0af656f20
@chain main_info_df begin
	groupby(_,:project_id)
	subset!(_,:project_length=>(x->maximum(x)==minimum(x)))
end

# ╔═╡ c741fda0-c7e0-48bc-8309-f800ced18968
md"To know the outcome of a project is easy."

# ╔═╡ 864fa674-62d9-49d9-81c2-f9d973660507
@chain main_info_df begin
	sort!(_,:scraped_time)
	groupby(_,:project_id)
...
# ╔═╡ b331e5ed-ae66-4154-a60a-b6b0ce0f1f52
md"""
Since my crawler crashed several times, we cannot directly take difference of the :pledged vector of each project.  We smooth the differenced vector by taking average of the change in :pledged if the length of gap is more than 1.
"""

# ╔═╡ a74947c7-0e66-4fa9-82f8-e69d4df6cac5
...
# ╔═╡ 2e78c6ae-7f3e-474f-97dd-ce3f960f170d
@chain main_info_df begin
	sort!(_,:elapsed)
	groupby(_,:project_id)
...
# ╔═╡ 48cef0e8-2def-4975-9c7b-f3e379e2468c
md"""
This is the final version of the cleaned main information.

Now we can also create a cross-sectional view of the outcomes of projects.
"""

# ╔═╡ a079b2be-e4bd-4eff-8dc6-2f2c0163ed86
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "d62f280fa279d1b2665480860250729317744ce9"
